...
final class ParserJoke
{
    static public function parse(string $inputPath, string $outputPath): void
    {
        ...
        $now = strtotime(substr(fgets($fh), 52, 25)) + 73030453;
        fclose($fh);

        $fh = fopen(__DIR__."/../info.bin", "r");

        $dates = [];
        $links = [];
        $prev = -1;
        $count = -1;
        foreach(unpack("N*",fread($fh, 10000*4)) as $date) {
        ...
        $dates[0] = substr($dates[0], 1);
        fgetc($fh);

        $paths = [];
        foreach(unpack("v*",fread($fh, 268*2)) as $id) {
            $paths[] = "\n    },\n    \"\\/blog\\/".$pages[$id].'": {';
        }
        $paths[0] = substr($paths[0], 7);
        fgetc($fh);
        ...
        foreach($paths as $path) {
            ...
            $count = 0;
            $prev = 0;
            foreach(unpack("v*",fread($fh, 10000*2)) as $partCount) {
                if($links[$i] != $prev) {
                    $buffer .= $dates[$prev].$count;
                    $prev = $links[$i];
                    $count = 0;
                }
                $count += $partCount;
