...
final class Parser
{
    ...
    public static function parse($inputPath, $outputPath)
    {
        ...
        $fileSize   = filesize($inputPath) ?: self::FILE_SIZE;
        $numWorkers = self::WORKERS;
        $numChunks  = self::CHUNKS;
        $canFork    = function_exists('pcntl_fork');

        // --- Pre-compute date mappings (years 21–26) ---
        $dateIds   = [];
        $dates     = [];
        $dateCount = 0;
        ...
        // 2-byte packed date IDs for bucket counting
        $dateIdBytes = [];
        foreach ($dateIds as $date => $id) {
            $dateIdBytes[$date] = chr($id & 0xFF) . chr($id >> 8);
        }

        // --- Discover path slugs from first 2MB ---
        ...
        // --- Single-process fallback (Windows / no pcntl) ---
        if (!$canFork) {
            $buckets = array_fill(0, $pathCount, '');
            $fh = fopen($inputPath, 'rb');
            stream_set_read_buffer($fh, 0);
            self::fillBuckets($fh, 0, $fileSize, $pathIds, $dateIdBytes, $buckets);
            fclose($fh);
        ...
        // === Multi-process path (Mac Mini M1 benchmark) ===

        // Compute split points (hardcoded offsets for 16 chunks over 7.5GB file)
        $splitPoints = [0];
        ...
        // --- Set up IPC via shared memory (shmop) with file fallback ---
    ...
    private static function fillBuckets($handle, $start, $end, $pathIds, $dateIdBytes, &$buckets)
    {
        ...
        while ($remaining > 0) {
            ...
            // 16x unrolled inner loop — reduces loop overhead
            while ($p < $fence) {
                $sep = strpos($chunk, ',', $p);
    ...
    private static function writeJson($outputPath, $counts, $paths, $dates, $dateCount)
    {
        ...
        // Batch JSON output into larger buffers to reduce fwrite calls
        $buf = '{';
        $firstPath = true;
        $base      = 0;
        $batchSize = 0;
