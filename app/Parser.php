...
final class Parser
{
    private const CHUNK_SIZE = 64 * 1024 * 1024;
    ...
    private function processSegment(string $path, int $start, int $end): array
    {
        ...
        while ($left > 0) {
            ...
            if ($carry !== '') {
                ...
                $line = $carry . substr($raw, 0, $nl);
                $carry = '';
                $len = strlen($line);
                if ($len >= 45) {
                    $url = substr($line, 19, $len - 45);
            ...
            while (($nl = strpos($raw, "\n", $offset)) !== false) {
                $url = substr($raw, $offset + 19, $nl - $offset - 45);
                $date = substr($raw, $nl - 25, 10);
                isset($visits[$url][$date]) ? $visits[$url][$date]++ : $visits[$url][$date] = 1;
                $offset = $nl + 1;
            }

            $carry = $offset < $rawLen ? substr($raw, $offset) : '';
        }
